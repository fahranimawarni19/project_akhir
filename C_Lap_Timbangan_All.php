...
$data = mysqli_query($con,"SELECT t.tgl_timbangan,  n.nama, s.jenis_sampah,  s.harga, d.berat,  (d.berat * s.harga) AS total_harga FROM tb_detail_timbangan d JOIN   tb_timbangan t ON d.id_timbangan = t.id_timbangan JOIN tb_nasabah n ON t.id_nasabah = n.id_nasabah JOIN tb_sampah s ON d.kode_sampah = s.kode_sampah ORDER BY  t.tgl_timbangan, n.nama");
